...
const firstJsonUrl = `http://127.0.0.1:9200/current_traffic_situation/_doc/1`;
...
async function downloadTrafficSituation() {
        ...
        if (element.region.id === 7 || element.region.id === 8) {
          ...
          if (element.location) {
            ...
            if (loc.polygons) {
              ...
              geometry.coordinates = coordinatesPoly;
            ...
            if (loc.lines) {
              ...
              geometry.coordinates = coordinatesMulti;
  ...
  if (firstJson == undefined || firstJson.length < 1) {
    ...
    try {
      ...
      obj.name = "Current_Traffic";
      obj.features = filteredResult;

      // fs.writeFileSync(
      //   `./Data/TrafficSituation/Traffic.json`,
      //   JSON.stringify(obj)
      // );
